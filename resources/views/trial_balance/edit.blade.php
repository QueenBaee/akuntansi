...
@section('title', 'Edit Trial Balance')

@section('page-header')
    <div class="page-pretitle">Master Data</div>
    <h2 class="page-title">Edit Trial Balance</h2>
@endsection
                ...
                <form action="{{ route('trial-balance.update', $trial_balance->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="kode" class="form-control" 
                               value="{{ old('kode', $trial_balance->kode) }}" required>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="keterangan" class="form-control" 
                               value="{{ old('keterangan', $trial_balance->keterangan) }}" required>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="level" class="form-control" required>
                            <option value="1" {{ $trial_balance->level == 1 ? 'selected' : '' }}>Level 1</option>
                            <option value="2" {{ $trial_balance->level == 2 ? 'selected' : '' }}>Level 2</option>
                            <option value="3" {{ $trial_balance->level == 3 ? 'selected' : '' }}>Level 3</option>
                            <option value="4" {{ $trial_balance->level == 4 ? 'selected' : '' }}>Level 4</option>
                        </select>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="parent_id" class="form-control">
                            ...
                            @foreach($parents as $p)
                                <option value="{{ $p->id }}" 
                                    {{ $trial_balance->parent_id == $p->id ? 'selected' : '' }}>
                                    {{ $p->kode }} - {{ $p->keterangan }}
                                </option>
                            @endforeach
                        </select>
