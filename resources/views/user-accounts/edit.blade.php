...
<div class="row">
    <div class="col-md-8">
        <div class="card">
            ...
            <form action="{{ route('user-accounts.update', $userAccount) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <select name="user_id" class="form-select @error('user_id') is-invalid @enderror" required>
                            ...
                            @foreach($users as $user)
                                <option value="{{ $user->id }}" {{ old('user_id', $userAccount->user_id) == $user->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="account_id" class="form-select @error('account_id') is-invalid @enderror" required>
                            ...
                            @foreach($accounts as $account)
                                <option value="{{ $account->id }}" {{ old('account_id', $userAccount->account_id) == $account->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="role" class="form-control @error('role') is-invalid @enderror" 
                               value="{{ old('role', $userAccount->role) }}" placeholder="e.g., owner, admin, member">
                        @error('role')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        <label class="form-check">
                            <input type="checkbox" name="is_active" value="1" class="form-check-input" 
                                   {{ old('is_active', $userAccount->is_active) ? 'checked' : '' }}>
                ...
                <div class="card-footer text-end">
                    <div class="d-flex">
                        ...
                        <button type="submit" class="btn btn-primary ms-auto">Update User Account</button>
                    </div>
                </div>
            </form>
        </div>
